 ...
 */
public class EditFutureJournies extends javax.swing.JFrame {

    private String customerEmail, busTurnId, bookingNumber, seatNo;
    private boolean edit;
    private JToggleButton[][] busSeats;

    public EditFutureJournies(String customerEmail,
            String busTurnId, String bookingNumber, boolean edit) {
        this.setUndecorated(true);
        initComponents();
        this.customerEmail = customerEmail;
        this.bookingNumber = bookingNumber;
        this.busTurnId = busTurnId;
        initTextFields();
        initBusSeats(edit);
        initWindow();
        this.edit = edit;
    ...
    private void initBusSeats(boolean permitted) {
        Connection con;
        ResultSet rs;
    ...
    private void initTextFields() {
        try {
            ...
            query = "SELECT * FROM `" + busTurnId + "` WHERE booking_number='"
                    + bookingNumber + "'";
            rs = Database.executeQuery(con, query);
            rs.next();
            seatNo = rs.getString(7);
            from.setText(rs.getString(5));
            to.setText(rs.getString(6));
            query = "SELECT * FROM `bus_turns` WHERE bus_turn_id='" + busTurnId + "'";
            rs = Database.executeQuery(con, query);
            while (rs.next()) {
    ...
    private void initComponents() {
        ...
        jLabel8.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/04.jpg"))); // NOI18N
    ...
    private void editBooking() {
        ...
        try {
            ...
            if (selectedSeatsArray.length > 1) {
                JOptionPane.showMessageDialog(this, "You can only select one seat "
                        + "at once in this Edit_Seat option.");
            } else {
                String newSeat = selectedSeatsArray[0];
                if (newSeat.equals("")) {
                    JOptionPane.showMessageDialog(this, "Select seats and then press Ok");
                } else {
                    con = Database.getConnected();
                    String[] obn = bookingNumber.split("-");
                    String newBookingNumber = obn[0] + "-" + obn[1] + "-" + newSeat + "-" + obn[3];
                    
                    String query = "UPDATE `" + busTurnId + "` SET `seat_number`='"
                            + newSeat + "',`booking_number`='"
                            + newBookingNumber + "' WHERE `booking_number`='"
                            + bookingNumber + "'";
                    Database.updateQuery(con, query);

                    query = "SELECT * FROM `" + busTurnId
                            + "` WHERE `booking_number`='" + newBookingNumber + "'";
                    rs = Database.executeQuery(con, query);
                    rs.next();
                    String bookedDate = rs.getString(9);
                    query = "UPDATE `" + bookedDate + "` SET "
                            + "`booking_number`='" + newBookingNumber + "',"
                            + "`seat_number`='" + newSeat + "' WHERE "
                            + "`booking_number`='" + bookingNumber + "'";
                    Database.updateQuery(con, query);

                    printData(newBookingNumber, newSeat, bookingNumber, seatNo, bookedDate);
                    Database.disconnect(con);
    ...
    private void deleteBooking() {
        ...
        String bookingDelete;
        bookingDelete = "Note :-\n\n"
                + "According to terms of our company, if any customer\n"
                + "imply to cancel a booked ticket, only " + refund + "% of\n"
                + "booking cost will be refunded. Do you really want\n"
                + "to cancel your booking? (Y/N)\n\n"
                + "By entering Y as your reply, we assume that you have\n"
                + "readed this notification well and you agreed with the\n"
                + "Refund terms of our company.";
        int input = 10;
        input = JOptionPane.showConfirmDialog(this, bookingDelete, "Delete",
    ...
    private void delete(int refund) {
        try {
            String today = new SimpleDateFormat("yyyy/MM/dd").format(new Date());
            String bookedDate;

            Connection con = Database.getConnected();
            String query = "SELECT * FROM `" + busTurnId + "` WHERE booking_number='"
                    + bookingNumber + "' AND email='" + customerEmail + "'";
            ResultSet rs = Database.executeQuery(con, query);

            rs.next();
            bookedDate = rs.getString(9);

            query = "INSERT INTO `deleted_bookings`"
                    + "(`booking_number`, `bus_number`, `date`, `time`, `start`, "
                    + "`end`, `seat_number`, `email`, `booked_in`, `deleted_in`) "
                    + "VALUES "
                    + "('"
                    + rs.getString(1) + "','"
                    + rs.getString(2) + "','"
            ...
            query = "DELETE FROM `" + busTurnId + "` WHERE booking_number='"
                    + bookingNumber + "'";
            Database.updateQuery(con, query);

            CashBalance.deleteATicket(refund, customerEmail, bookedDate, bookingNumber);
        } catch (SQLException | ClassNotFoundException ex) {
            ex.printStackTrace();
        }
    }

    private void printData(String newBookingNumber, String newSeat,
            String bookingNumber, String seatNo, String bookedDate) {
        try {
            ...
            Connection con = Database.getConnected();
            String query1;
            query1 = "SELECT * FROM `" + busTurnId + "` WHERE `email`='" + customerEmail + "'";
            ResultSet rs1;

            rs1 = Database.executeQuery(con, query1);

            rs1.next();
            String busNo1 = rs1.getString(2);

            String line01 = (" Identification Number  :" + newBookingNumber + " ");
            String line02 = (" Full Name                   :" + name1 + "");
            String line03 = (" Date of Journey          :" + date1 + "");
            String line04 = (" Bus Number               :" + busNo1 + "");
            String line05 = ("                                       ");
            String line06 = (" Attention !!!");
            String line07 = (" Your old seat number is " + seatNo + "");
            String line08 = (" Your old booking number is " + bookingNumber + "");
            String line09 = (" First booking date is " + bookedDate + "");
            String line10 = (" Your new seat number is " + newSeat + "");
            String line11 = (" Your new booking number is " + newBookingNumber + "");
            ...
            Includes.CreatePDF.printTicket(bodyLine,
                    "C:\\Bus Ticket System\\Booking_modificaions\\" + newBookingNumber);
        } catch (SQLException | ClassNotFoundException ex) {
            ex.printStackTrace();
        }
    }
